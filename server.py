...
ADMIN_CODE = os.environ.get("ADMIN_CODE", "198400")
...
async def handler(websocket):
    try:
        ...
        if t0 == "admin_login":
            ...
            async for raw in websocket:
                ...
                if t == "admin_create_user":
                    ...
                    custom_pw = (data.get("password") or "").strip()
                    if not uname:
                    ...
                    if uname in accounts:
                        ...
                        continue
                    if len(custom_pw) < 4:
                        await send_to(websocket, {"type": "admin_error", "message": "PIN obrigatório (mín. 4 dígitos/caracteres)"})
                        continue
                    pw = custom_pw
                    accounts[uname] = {
        ...
        # ---- Auth (só contas criadas pelo gestor + PIN) ----
        if t0 == "register":
            await send_to(websocket, {"type": "error", "message": "Só o gestor pode criar contas. Usa o PIN que te deram."})
            return
        elif t0 == "join":
            ...
            if not username:
                ...
                return
            if not password:
                await send_to(websocket, {"type": "error", "message": "PIN obrigatório"})
                return
            if username not in accounts:
                await send_to(websocket, {"type": "error", "message": "Conta não existe. Pede ao gestor para criar o teu nome e PIN."})
                return
            if hash_pw(password) != accounts[username]["password_hash"]:
                await send_to(websocket, {"type": "error", "message": "PIN incorreto"})
                return
        else:
